...
// - 8SPINE module: added retry logic + error handling in getTrackStreamUrl
// - 8SPINE module: added prefetchTrack for next-track preloading
// - 8SPINE module: better track object with audioQuality mapping
...
function getApiKey(env) {
  return env?.YTM_API_KEY || YTM_API_KEY_FALLBACK;
}
...
async function fetchFreshVisitorData(env, userToken) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (vd) {
        upstashCmd(env, 'SET', key, vd, 'EX', VISITOR_TTL_SEC);
        return vd;
      }
...
async function ytDataChannelVideos(channelId, artistName, env) {
  ...
  try {
    ...
    if (!dr.ok) return videoIds.map((id, i) => ({
      id, title: data.items[i]?.snippet?.title || 'Unknown', artist: artistName, album: '',
      duration: 0, artworkURL: upscaleArtwork(data.items[i]?.snippet?.thumbnails?.high?.url || ''), format: 'aac',
    }));
...
function extractChannelVideoTracks(data, fallbackArtist) {
  ...
  const pushVideo = (videoId, title, durText, thumbs) => {
    ...
    seenIds.add(videoId);
    tracks.push({
      id: videoId, title, artist: fallbackArtist || '', album: '',
      duration: dur, artworkURL: bestThumbnail(thumbs) || '', format: 'aac'
    });
  };
  ...
  for (const tab of allTabs) {
    ...
    if (richGrid) {
      for (const item of richGrid.contents || []) {
        ...
        if (vr) {
          const durText = vr.lengthText?.simpleText || vr.lengthText?.accessibility?.accessibilityData?.label || vr.thumbnailOverlays?.find(o => o.thumbnailOverlayTimeStatusRenderer)?.thumbnailOverlayTimeStatusRenderer?.text?.simpleText || '';
...
function parseTrackRenderer(r, fallbackArtist, fallbackAlbum, fallbackArtwork) {
  ...
  const durationStr = (isDuration(fixedRaw) ? fixedRaw : '') || info.duration || (r.lengthMs ? `${Math.floor(r.lengthMs / 60000)}:${String(Math.floor((r.lengthMs % 60000) / 1000)).padStart(2, '0')}` : '');
...
function parseAlbumItem(item) {
  ...
  if (r2) {
    ...
    const id = r2.navigationEndpoint?.browseEndpoint?.browseId || r2.overlay?.musicItemThumbnailOverlayRenderer?.content
  ...
  if (r) {
    ...
    const id = r.navigationEndpoint?.browseEndpoint?.browseId || r.overlay?.musicItemThumbnailOverlayRenderer?.content
      ?.musicPlayButtonRenderer?.playNavigationEndpoint?.browseEndpoint?.browseId ||
      (r.flexColumns?.[0]?.musicResponsiveListItemFlexColumnRenderer?.text?.runs || [])
...
async function handleSearch(query, env, userToken, mode) {
  ...
  if (songsR.status === 'fulfilled' && songsR.value)
    for (const s of getShelves(songsR.value)) for (const it of s.contents || []) {
      addTrack(parseTrackRenderer(it.musicResponsiveListItemRenderer));
      if (tracks.length >= 40) break;
    }
  if (videosR.status === 'fulfilled' && videosR.value)
    for (const s of getShelves(videosR.value)) for (const it of s.contents || []) {
      addTrack(parseTrackRenderer(it.musicResponsiveListItemRenderer));
      if (tracks.length >= 100) break;
    }
  if (fetchAlbums && albumsR.status === 'fulfilled' && albumsR.value)
    for (const s of getShelves(albumsR.value)) for (const it of s.contents || []) {
      const a = parseAlbumItem(it);
      if (a && albums.length < 15) albums.push(a);
    }

  const seenArtistIds = new Set();
  if (artistsR.status === 'fulfilled' && artistsR.value)
    for (const s of getShelves(artistsR.value)) for (const it of s.contents || []) {
      const a = parseArtistItem(it);
      if (a && !seenArtistIds.has(a.id) && artists.length < 12) {
    ...
    for (const sec of tabs) {
      ...
      if (id && id.startsWith('UC') && !seenArtistIds.has(id) && artists.length < 12) {
        ...
        if (name) {
          ...
          artists.push({ id, name, artworkURL });
        }
      }
    }
  }
  if (plR.status === 'fulfilled' && plR.value)
    for (const s of getShelves(plR.value)) for (const it of s.contents || []) {
      const p = parsePlaylistItem(it);
      if (p && playlists.length < 12) playlists.push(p);
    }
  ...
  if (needTracks) {
    try {
      ...
      for (const s of getShelves(tFB)) for (const it of s.contents || []) {
        addTrack(parseTrackRenderer(it.musicResponsiveListItemRenderer));
        if (tracks.length >= 40) break;
      }
    } catch (e) {
      console.log(LOG_PREFIX, 'tracks backfill failed:', e.message);
    }
  }
  if (needAlbums) {
    try {
      const aFB = await ytmSearchAndroid(query, SEARCH_PARAMS.albums, env);
      for (const s of getShelves(aFB)) for (const it of s.contents || []) {
        const a = parseAlbumItem(it);
        if (a && albums.length < 15) albums.push(a);
      }
    } catch (e) {
      console.log(LOG_PREFIX, 'albums backfill failed:', e.message);
    }
  }
  if (needArtists) {
    try {
      ...
      console.log(LOG_PREFIX, 'artists backfill failed:', e.message);
    }
  }
  if (needPlaylists) {
    try {
      const pFB = await ytmSearchAndroid(query, SEARCH_PARAMS.playlists, env);
      for (const s of getShelves(pFB)) for (const it of s.contents || []) {
        const p = parsePlaylistItem(it);
        if (p && playlists.length < 12) playlists.push(p);
      }
    } catch (e) {
      console.log(LOG_PREFIX, 'playlists backfill failed:', e.message);
    }
  }
  ...
  if (artists.length === 0) {
    ...
    for (const ch of ytChannels)
      if (!seenArtistIds.has(ch.id) && artists.length < 12) {
        seenArtistIds.add(ch.id);
        artists.push(ch);
      }
  }
...
async function fetchPlayerDataWebCreator(trackId, env) {
  const baseBody = {
    context: { client: WEB_CREATOR_CLIENT, thirdParty: { embedUrl: `https://www.youtube.com/embed/${trackId}` } },
    videoId: trackId,
    contentCheckOk: true,
    racyCheckOk: true
  };
...
async function fetchPlayerDataWebEmbedded(trackId, env) {
  const baseBody = {
    context: { client: WEB_EMBEDDED_CLIENT, thirdParty: { embedUrl: `https://www.youtube.com/embed/${trackId}` } },
    videoId: trackId,
    contentCheckOk: true,
    racyCheckOk: true
  };
...
// ── FIX: Stream URL caching + reordered resolvers ──────────────────────
// Cache resolved stream URLs in Redis so repeated plays are instant.
// Reorder: direct audio formats (m4a/mp4) first, HLS last.
// 8SPINE prefers direct audio URLs per the module spec — HLS can cause
// delays because the player has to fetch and parse the manifest first.
...
async function handleAlbum(albumId, env, userToken) {
  ...
  if (!albumArtist) {
    ...
    for (const run of hdr.subtitle?.runs || []) {
      ...
      if (t && !isBullet(t) && !skip.has(t) && !/^\d{4}$/.test(t)) {
        albumArtist = t;
        break;
      }
    }
...
async function handleArtist(artistId, env, userToken, mode) {
  ...
  if (mode === 'videos') {
    ...
    try {
      ...
      console.log(LOG_PREFIX, 'channel browse failed:', e.message);
    }
    if (topTracks.length === 0 && env?.YOUTUBE_DATA_API_KEY) {
      try {
        topTracks = await ytDataChannelVideos(artistId, name, env);
      } catch (e) {
        console.log(LOG_PREFIX, 'ytData videos fallback failed:', e.message);
      }
    }
    if (topTracks.length === 0) {
      try {
        ...
        console.log(LOG_PREFIX, 'video search fallback failed:', e.message);
      }
    }
  ...
  if (topTracks.length === 0) {
    try {
      ...
      console.log(LOG_PREFIX, 'artist top-tracks fallback failed:', e.message);
    }
  }
  ...
  if (topTracks.some(t => t.duration === 0)) {
    try {
      ...
      console.log(LOG_PREFIX, 'duration enrich failed:', e.message);
    }
  }
...
function buildManifest(mode) {
  ...
  const variants = {
    both: {
      ...
      name: 'YouTube Music',
      description: 'Stream from YouTube Music — Songs & Videos, Albums, Artists, Playlists. HLS + AAC.'
    },
    songs: {
      id: 'com.ricky.youtube-music-songs',
      name: 'YouTube Music — Songs',
      description: 'Stream from YouTube Music — Songs tab only. Albums, Artists & Playlists. HLS + AAC.'
    },
    videos: {
      id: 'com.ricky.youtube-music-videos',
      name: 'YouTube Music — Videos',
      description: 'Stream from YouTube Music — Videos tab. Artists & Playlists. HLS + AAC.'
    },
  };
...
// - Error handling that doesn't break the player
// ═══════════════════════════════════════════════════════════════════════
...
async function handleRoute(rest, url, request, env, userToken, mode) {
  ...
  if (rest.startsWith('/stream/')) {
    ...
    return jsonRes(await handleStream(id, env, userToken));
  }
  if (rest.startsWith('/download/')) {
    const id = lastSegment(rest);
    if (!id) return jsonRes({ error: 'Missing ID' }, 400);
    return await proxyDownload(id, request, env, userToken);
  }
  if (rest.startsWith('/album/')) {
    const id = lastSegment(rest);
    if (!id) return jsonRes({ error: 'Missing ID' }, 400);
    return jsonRes(await handleAlbum(id, env, userToken));
  }
  if (rest.startsWith('/artist/')) {
    const id = lastSegment(rest);
    if (!id) return jsonRes({ error: 'Missing ID' }, 400);
    return jsonRes(await handleArtist(id, env, userToken, mode));
  }
  if (rest.startsWith('/playlist/')) {
    const id = lastSegment(rest);
    if (!id) return jsonRes({ error: 'Missing ID' }, 400);
    return jsonRes(await handlePlaylist(id, env, userToken));
  }
  return null;
}
...
function buildPage() {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>YouTube Music — Eclipse Addon</title><style>body{font-family:system-ui,sans-serif;max-width:680px;margin:40px auto;padding:0 20px;background:#0f0f0f;color:#f1f1f1;line-height:1.6}a{color:#3ea6ff}code{background:#1f1f1f;padding:2px 6px;border-radius:4px;font-size:0.9em}h1{font-size:1.5em}</style></head><body><h1>YouTube Music — Eclipse Addon</h1><p>Full YouTube Music catalog — Songs, Videos, Albums, Artists & Playlists. No account required.</p></body></html>`;
}
...
export default {
  async fetch(request, env) {
    ...
    let userToken = null, rest = path, mode = 'both';
    ...
    if (parsed) {
      ...
      if (m === 'songs' || m === 'videos') mode = m;
    }

    try {
      const result = await handleRoute(rest, url, request, env, userToken, mode);
      if (result) return result;
      return jsonRes({ error: 'Not found', path: rest }, 404);
    } catch (e) {
